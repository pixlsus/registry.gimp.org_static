...
;;; Version 0.7
;;; 2010 Oct 16 - 
;;; 1) Modified to allow an offset from the center
...
(define (script-fu-line-nova img drw num-of-lines corn-deg offset variation xoffset yoffset)
  (let* (
    ...
    (while (< index num-of-lines)
      (draw-vector (+ (nth 0 drw-offsets) xcenterOffset)
                   (+ (nth 1 drw-offsets) ycenterOffset)
                   (* index dir-deg/line)
      )
      (set! index (+ index 1))
    )
...
(script-fu-register "script-fu-line-nova"
  _"Line _Nova..."
  _"Fill a layer with rays emanating outward from its center offset by x,y using the FG color"
  ...
  SF-ADJUSTMENT _"Number of lines"     '(200 3 1000 1 1 0 1)
