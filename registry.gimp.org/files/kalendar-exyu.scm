...
; ------------------------------------------------------------------------------
; Original version by Saul Good
; http://chiselapp.com/user/saulgoode/repository/script-fu/wiki?name=sg-calendar
;
; Modified by SkYNeT on January 2013.
; Translated user interface and added Ex-Yu translations for months and days.
; (Serbian latin, Serbian cyrilic, Croatian, Slovenian and Macedonian)
;
; Modified version does not conflict with the original version.
; You can use both versions without problems.
;
; Modified version is in menu: Filters -> SkYNeT -> Render ->
;                                                   Kalendar - godisnji...
;                                                   and
;                                                   Kalendar - mesecni...
;
; All credits go to Saul Good
; ------------------------------------------------------------------------------
;
; ------------------------------------------------------------------------------
; Originalnu verziju je napravio Saul Good
; http://chiselapp.com/user/saulgoode/repository/script-fu/wiki?name=sg-calendar
;
; Izmenio SkYNeT u Januaru 2013.
; Preveden korisnicki interfejs na srpski jezik
; i prevedeni nazivi za mesece i dane na Ex-Yu jezike.
; (srpski latinica, srpski cirilica, hrvatski, slovenacki i makedonski)
;
; Izmenjena verzija nema konflikte sa originalnom verzijom.
; Mogu se koristiti obe verzije bez problema.
;
; Modifikovana verzija je u meniju: Filters -> SkYNeT -> Render ->
;                                                        Kalendar - godisnji...
;                                                        i
;                                                        Kalendar - mesecni...
;
; Sve zasluge oko programiranja za Saul Good
; ------------------------------------------------------------------------------
...
(define kalendar-exyu-languages '("Srpski" "Српски" "Hrvatski" "Slovensko" "Македонски") )

(define kalendar-exyu-months '(
                  ; Srpski
...
;
(define (kalendar-exyu-calc-fontsize text font fontsize% width height)
...
(define (script-fu-kalendar-exyu image drawable lang month year sunday? letters-in-day layout text-font number-font fontsize% justify? border border-color gravity)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (let* ((bg-layer 0)
         ...
         (orig-sel 0)
         (months (list->vector (list-ref kalendar-exyu-months lang)))
         (weekday-strings (list->vector (map (lambda (x) 
                                               (substring x 
                                                          0 
                                                          (min (+ letters-in-day 1)
                                                               (string-length x) ) ) )
                                             (list-ref kalendar-exyu-weekdays lang) )))
         )         
    (gimp-context-push)
    ...
    (set! months (list->vector (list-ref kalendar-exyu-months lang)))
    ...
    (set! text-fontsize (apply min (map (lambda (text)
                               (kalendar-exyu-calc-fontsize text 
                                              text-font 
                                              80 
    ...
    (if (< fontsize% 80)
      (set! number-fontsize (kalendar-exyu-calc-fontsize "30" number-font fontsize% (- (cadr X's) (car X's) (* 4 border))  (- (cadr Y's) (car Y's) (* 4 border))))
      (set! number-fontsize (kalendar-exyu-calc-fontsize "30" number-font fontsize% (- (cadr X's) (car X's) (* 2 border))  (- (cadr Y's) (car Y's) (* 2 border))))
      )
...
(define (script-fu-kalendar-exyu-year orig-image orig-drawable 
                                    lang 
                                    year
                                    start-month
                                    end-month
  ...
  (let* ((image 0)
    ...
    (let* ((num-months (succ (modulo (- end-month start-month) 12)))
      ...
      (set! month-fontsize (apply min (map (lambda (text) 
                                               (kalendar-exyu-calc-fontsize text text-font 100 w-sel (- h h-sel)) )
                                           (list-ref kalendar-exyu-months lang) )))
      (set! y (+ (cadr (gimp-text-get-extents-fontname (car (list-ref kalendar-exyu-months lang)) 
                                                       month-fontsize 
                                                       PIXELS 
                                                       text-font ))
                 2) )
      ...
      (while (pair? y-offsets)
        ...
        (while (and (pair? x-offsets) (< month-index num-months))
          ...
          (let ((month (modulo (+ start-month month-index) 12)))
            (script-fu-kalendar-exyu image temp-layer lang month year sunday? 
            ...
            (set! extents (gimp-text-get-extents-fontname (list-ref (list-ref kalendar-exyu-months lang) month) 
                                                          month-fontsize 
                                                          PIXELS 
                                                          text-font ))
            (set! temp-layer (car (gimp-text-fontname image -1 
                                                      ...
                                                      (list-ref (list-ref kalendar-exyu-months lang) month) 
                                                      0 TRUE 
...
(script-fu-register "script-fu-kalendar-exyu"
  "Kalendar - mesecni..."
  "Generise mesecni kalendar za izabrani lejer"
  "Saul Goode"
  "Saul Goode"
  ...
  SF-OPTION "Izbor jezika" kalendar-exyu-languages
  SF-OPTION "Mesec" (car kalendar-exyu-months)
  SF-ADJUSTMENT "Godina" '( 2012 1753 2050 1 10 0 1 )
  SF-TOGGLE "Nedelja prvi dan" FALSE
  SF-OPTION "Format dana" '("P U S ..." "Po Ut Sr ..." "Pon Uto Sre ...")
  SF-OPTION "Sablon" '( "Dozvoliti prikaz u 6 nedelja" "Uvek prikaz u 6 nedelja" "Prebaciti 6. nedelju u 1. nedelju" "Prebaciti 6. nedelju u 5. nedelju")
  SF-FONT "Font teksta" "Sans" 
  SF-FONT "Font brojeva" "Sans" 
  SF-ADJUSTMENT "Velicina fonta (%)" '( 100 0 100 1 10 0 1)
  SF-TOGGLE "Desno ravnanje" TRUE
  SF-ADJUSTMENT "Sirina okvira" '( 1 0 5 1 1 0 1 )
  SF-COLOR "Boja okvira" '(0 0 0)
  SF-OPTION "Pozicija datuma" '( "gore-levo" "gore-sredina" "gore-desno" "levo-sredina" "sredina" "desno-sredina" "dole-levo" "dole-sredina" "dole-desno")
  )
(script-fu-menu-register "script-fu-kalendar-exyu"
  "<Image>/Filters/SkYNeT/Render"
  )


(script-fu-register "script-fu-kalendar-exyu-year"
  "Kalendar - godisnji..."
  "Generise godisnji kalendar za izabrani lejer"
  "Saul Goode"
  "Saul Goode"
  ...
  SF-DRAWABLE "Drawable"  0
  SF-OPTION "Izbor jezika" kalendar-exyu-languages
  SF-ADJUSTMENT "Godina" '( 2013 1753 2050 1 10 0 1 )
  SF-OPTION "Pocetni mesec" (car kalendar-exyu-months)
  SF-OPTION "Zavrsni mesec" (cons (car (last (car kalendar-exyu-months))) (butlast (car kalendar-exyu-months)))
  SF-ADJUSTMENT "Kolone" '( 4 1 12 1 10 0 1 )
  SF-ADJUSTMENT "Razmak" '( 10 0 80 1 10 0 1 )
...
(script-fu-menu-register "script-fu-kalendar-exyu-year"
  "<Image>/Filters/SkYNeT/Render"
  )
