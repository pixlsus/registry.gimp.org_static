...
;;; Version 0.7
;;; 2012 May 06 -
;;; 1) Modified GIMP 2.8.0 script to allow an offset from the center
;;; 2) Changed "Number of lines" lower limit from 40 to 3

(define (script-fu-line-nova img drw num-of-lines corn-deg offset variation xoffset yoffset)
  (let* (
        ...
        (xcenter (/ drw-width 2))
        (ycenter (/ drw-height 2))
        (xcenterOffset (+ xcenter xoffset))
        (ycenterOffset (+ ycenter yoffset))
        )
    (gimp-context-push)
    ...
    (while (< index num-of-lines)
      (draw-vector (+ (nth 0 drw-offsets) xcenterOffset)  ; was (/ drw-width 2)
                   (+ (nth 1 drw-offsets) ycenterOffset)  ; was (/ drw-height 2)
                   (* index dir-deg/line)
      )
      (set! index (+ index 1))
    )
...
(script-fu-register "script-fu-line-nova"
  _"Line _Nova..."
  _"Fill a layer with rays emanating outward from its center offset by x,y using the foreground color"
  ...
  SF-ADJUSTMENT _"Number of lines"     '(200 3 1000 1 1 0 1) ; was 40
  ...
  SF-ADJUSTMENT _"X Offset from center"       '(0 -4000 4000 1 1 0 1)
  SF-ADJUSTMENT _"Y Offset from center"       '(0 -4000 4000 1 1 0 1)
)
