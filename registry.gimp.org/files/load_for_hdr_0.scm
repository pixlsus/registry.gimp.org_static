...
(script-fu-register
	   ...
	   for HDR blending the whole image."
          ...
          SF-ADJUSTMENT "Exposure mask blur radius" '(20 0 1000 1 50 0 SF-SPINNER)
...
;  blur-radius       -  amount of gaussian blur to apply to the layer masks
...
(define (process-single-image 
    ...
    (if (not (= 0 blur-radius))
        (plug-in-gauss-iir RUN-NONINTERACTIVE source-image (car (gimp-image-get-active-drawable source-image)) blur-radius 1 1) )
...
(define (load-3-hdr curr-image dark-file bright-file blur-radius desaturate-option invert-option)
    (let* (
        (dark-image   (car (gimp-file-load 0 dark-file dark-file)))
        (bright-image (car (gimp-file-load 0 bright-file bright-file)))

        (layer0 (process-single-image curr-image dark-image "dark layer" desaturate-option 0 blur-radius))
        (layer1 (process-single-image curr-image bright-image "bright layer" desaturate-option (+ 1 invert-option) blur-radius))
       )
...
(define (load-5-hdr curr-image dark-file darkish-file brightish-file bright-file blur-radius desaturate-option invert-option)
    (let* (
        (dark-image   (car (gimp-file-load 0 dark-file dark-file)))
        ...
        (layer0 (process-single-image curr-image dark-image "dark layer" desaturate-option 0 blur-radius))
        (layer1 (process-single-image curr-image darkish-image "dark-ish layer" desaturate-option 0 blur-radius))
        (layer2 (process-single-image curr-image brightish-image "bright-ish layer" desaturate-option (+ 1 invert-option) blur-radius ))
        (layer3 (process-single-image curr-image bright-image "bright layer" desaturate-option (+ 1 invert-option) blur-radius))
       )
