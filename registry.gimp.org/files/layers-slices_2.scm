...
;
; --- v0.4
; Support for layer groups in gimp2.8
;
; --- v0.3
...
(define (delete-layer image)
	(let* (
					...
					(num-layers (- (car (gimp-image-get-layers image)) 1 ))	; Layer number
				)
				
		(delete-layer-group image layers num-layers)

	)
)

(define (delete-layer-group image layers num-layers)
	(let* (
					(name "")
					(layer 0)
					(first TRUE)
		...
		(while (>= i 0)
			(let* (
							...
							(group (car (gimp-item-is-group layer)))
						)
				(set! i (- i 1))

				(if (= group 1)
					(let* (
								(group-layers (cadr (gimp-item-get-children layer)))
								(group-num-layers (- (car (gimp-item-get-children layer)) 1 ))		; Number of layers
							)
						(delete-layer-group image group-layers group-num-layers)
					)
...
(define (manage-layers layers-list image param-directory param-jpg-quality)
	(let* (
					(layer 0)
					(flatten-layer 0)
					(name "")
					(layers (cadr layers-list))
					(i (- (car layers-list) 1))
				)
		...
		(while (>= i 0)
			(let* (
							...
							(name (car (gimp-drawable-get-name layer)))		; retrieve it's name
							(group (car (gimp-item-is-group layer)))
						)
				...
				(if (= group 1)
					(let* (
								(group-layers (gimp-item-get-children layer))
							)
						(manage-layers group-layers image param-directory param-jpg-quality)
					)
...
; start the recursive process through the layers tree
(define (manage-images image param-directory param-jpg-quality)
	(let* (
					(layers (gimp-image-get-layers image))
				)
		(manage-layers layers image param-directory param-jpg-quality)
